L.LayerGroup.StationsLayer = L.LayerGroup.extend({
  ...
  requestRegionData: function () {
    ...
    (function () {
      ...
      var stations = parsed_stations[state];
      var station_string = stations.slice(1, 10).join(",");
  ...
  getMarker: function (data) {
    ...
    if (isNaN(aqi)) {
      ...
      // Parsing AQI to see what color to use
    ...
    var defaultMarker = L.marker([lat, lon], {
      icon: L.divIcon({
        className: clName,
        iconSize: [36, 25],
        iconAnchor: [18, 40],
    ...
    var minimalMarker = L.circleMarker(L.latLng([lat, lon]), {
      radius: 5,
      weight: 1,
      fillOpacity: 1,
      color: "#7c7c7c",
    ...
    marker =
      this._map && this._map._minimalMode ? minimalMarker : defaultMarker;
    return marker;
  },
    ...
    (function (w, d, t, f) {
      w[f] =
        ...
        function (c, k, n) {
          ...
          s["n"] = n = n || 0;
          (Apa = d.createElement(t)), (e = d.getElementsByTagName(t)[0]);
          Apa.async = 1;
          Apa.src =
            "http://feed.aqicn.org/feed/" +
            c[n].city +
            "/" +
            (c[n].lang || "") +
            "/feed.v1.js?n=" +
            n +
            k;
          e.parentNode.insertBefore(Apa, e);
        };
    })(window, document, "script", "_aqiFeed");
    /* jshint ignore:end */

    marker.bindPopup(function () {
      // Fetch popup content only when clicked; else the quota will be reached
      ...
      $.getJSON(stationURL, function (stationData) {
        ...
        for (var species in stationData.data.iaqi) {
          strContent +=
            ...
            labels[species] +
            "</strong>: " +
            stationData.data.iaqi[species].v +
            ";<br>";
        }
        strContent +=
          "See <a href=" +
          stationData.data.city.url +
          ">AQICN - " +
          stationData.data.city.name +
          "</a> for more info. Data provided by aqicn.org.<br>From the <a href=https://github.com/publiclab/leaflet-environmental-layers/pull/79>AQICN Inventory</a> (<a href = https://publiclab.org/notes/sagarpreet/06-06-2018/leaflet-environmental-layer-library?_=1528283515>info</a>)";
        ...
        _aqiFeed({
          display: name + "%details <br>" + strContent,
